...
action = input('Press 1 to encode, press 2 to decode: ')
...
if action == '1':
    ...
    # Извлекаем биты сообщения из младших битов пикселей
    message_bits = ''
    for i in range(channel.size):
        pixel = channel.flat[i]
        pixel_bits = format(pixel, '08b')
        message_bits += pixel_bits[-1]

    # Преобразуем биты сообщения в текст
    message = ''
    for i in range(0, len(message_bits), 8):
        byte = int(message_bits[i:i+8], 2)
        message += chr(byte)

    # Извлекаем сообщение между маркерами
    start_index = message.find('START') + len('START')
